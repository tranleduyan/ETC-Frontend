  ...
  */
  ...
  const IsValidName = () => {
    ...
    // Else if first name is not in a valid form.
      ...
      currentPromptState >= 1 &&
      !REGEX.name.test(userInformation.firstName)
    ) {
      setIsError(true);
    ...
    // Else if last name is not in valid form.
      ...
      currentPromptState >= 1 &&
      !REGEX.name.test(userInformation.lastName)
    ) {
      setIsError(true);
    ...
    // Else if middle name is too long (we only take up to 30 referring to the schema)
      ...
      currentPromptState >= 1 &&
      userInformation.middleName.length > 30
    ) {
      setIsError(true);
  ...
  const SendVerification = async () => {
    ...
    axios
      .post(
        `${API.domain}/api/authentication/send-verification-code`,
        requestBody,
        {
          headers: {
            "X-API-KEY": API.key,
        ...
        <div className="SignUpPage-Form">
          ...
          <StandardTextInputField
            ...
            className={`${SetCurrentLastInputFieldClass(
              0
            )} SignUpPage-StandardTextInputField`}
            name="emailAddress"
